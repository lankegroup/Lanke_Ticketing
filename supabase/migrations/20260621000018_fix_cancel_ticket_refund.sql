-- Fix cancel ticket and refund preview functions
-- Fix: get_cancel_preview missing error handling and success flag check
-- Fix: cancel_ticket function signature mismatch
...
BEGIN
  ...
  IF (v_penalty_info->>'success')::BOOLEAN = false THEN
    RETURN jsonb_build_object('success', false, 'error', 'penalty_calculation_failed', 'message', '计算退票费失败');
  END IF;
...
DROP FUNCTION IF EXISTS public.get_cancel_preview(uuid);
