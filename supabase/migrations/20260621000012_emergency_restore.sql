...
-- 第二步：简化 get_seat_map 函数，使用原始可靠版本
-- 使用 EXISTS 子查询而不是 LEFT JOIN，避免 RLS 问题
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM seats WHERE id = p_seat_id) THEN
...
-- 第五步：简化 book_ticket_with_seat 函数
