-- Remove reschedule limit from admin_reschedule_seat so admins can reschedule multiple times
-- The RPC will now return a 'reschedule_warning' flag when count >= 1 so the UI can prompt
