...
BEGIN
  ...
  IF p_user_id IS NOT NULL THEN
    ...
    UPDATE user_balances SET balance = balance - v_total_amount WHERE user_id = p_user_id;

    INSERT INTO balance_transactions (user_id, type, amount, description, related_session_id)
    VALUES (p_user_id, 'payment', v_total_amount, '购票消费', p_session_id);
  END IF;
...
BEGIN
  ...
  IF p_user_id IS NOT NULL THEN
    ...
    INSERT INTO balance_transactions (user_id, type, amount, description, related_session_id, related_seat_id)
    VALUES (p_user_id, 'payment', v_total_amount, '购票消费', p_session_id, p_seat_id);
  END IF;
...
BEGIN
  ...
  IF v_reg.user_id IS NOT NULL THEN
    UPDATE user_balances SET balance = balance + v_total_amount WHERE user_id = v_reg.user_id;

    INSERT INTO balance_transactions (user_id, type, amount, description, related_registration_id)
    VALUES (v_reg.user_id, 'refund', v_total_amount, '退票退款', p_registration_id);
  END IF;
